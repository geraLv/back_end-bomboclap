...
const AuthUser_1 = require("../domain/entities/AuthUser");
...
function makeEmail(name, last, idx) {
...
async function waitForUser(usersRepo, userId, maxRetries = 30, intervalMs = 250) {
...
async function main() {
    ...
    const authRepo = new AuthUser_repo_1.AuthUserRepository();
    const usersRepo = new User_repo_1.UsersRepository();
    let created = 0;
    let failed = 0;
    for (let i = 1; i <= total; i++) {
        ...
        const password = `Passw0rd!${Math.floor(Math.random() * 1000000)}`;
        const authUser = new AuthUser_1.AuthUser(email, password, {
            role: "user",
            name,
            last_name: last,
            phone,
